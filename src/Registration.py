...
class CNN(object):
    ...
    def register(self, IX, IY):
        ...
        # identify background pixels, project code
        ...
        del DX1, DY1, DX2, DY2, DX3, DY3, PD1, PD2, PD3, z
        ...
        # localised radial matching with radius 7, project code
        g = []

        for i in range(784):
            yloc = np.array([i // 28, i % 28])
            j = C_all[i][1]
            xloc = np.array([j // 28, j % 28])
            if np.linalg.norm(yloc - xloc) < 7:
                g.append(i)

        C_all, quality = C_all[g], quality[g]
        # end of localised matching section

        tau_max = np.max(quality)
        while np.where(quality >= tau_max)[0].shape[0] <= 50: tau_max -= 0.01
