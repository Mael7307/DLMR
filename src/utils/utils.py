...
def tps_warp(Y, T, Y_image, out_shape, ylm):
    ...
    # create coordinate vectors to span the entire moving img
    ...
    out_image = np.full(out_shape, [255,255,255], dtype='uint8')

    out_image[iT, jT, :] = Y_image[iY, jY, :]

    temp1 = cv2.hconcat([jY, iY])
    temp2 = cv2.hconcat([iT, jT])
    index = []

    # determine new locations of landmark points
    for i in range(ylm.shape[0]):
        j = cKDTree(temp1).query(ylm[i], k=1)[1]
        index.append(j)
